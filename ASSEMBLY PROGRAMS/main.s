...
.extern merge_main
.text
@@@ MAIN FUNCTION FOR THE MERGE FUNCITON @@@@@
...
bl print
bl input_number
mov r8 , r0  @@@ storing second list length in r8
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
add sp , sp , #4
bl merge_main  @@@ main call 
ldr r2 , =output_prompt2
bl print
mov r0 , r7 @@@ moving the length of the merged list to r0 
bl output_number
bl output_new_line
...
swi 0x123456
@@@@@@@@@@@@@@@@@@@@@@@@@@@
@@@@@@@@@@@@@@@@@@@@@@@@@@ prints a string address being in r2 expects null termination 
print: mov r0 , #5 
ldr r3, =params 
mov r1 , #1
str r1 , [r3]
...
K: mov pc , lr 
@@@@@@@@@@@@@
@@@2  r0 for numbers , r1 address of list pointer , r2 string pool to actually store the strings 
read_list:
      cmp r0 , #0 
      moveq pc , lr 
      ...
      bal read_list 
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
@@@@@@@@@@@@ expects address to store string in r2 , reads all characters till enter and makes the string null terminated, after return r2 holds the next free address location 
